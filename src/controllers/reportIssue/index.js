...
reportIssue.enter(async (ctx) => {
  try {
    ...
    ctx.session.issueData = {
      description: '',
      // ⚠️ Храним только base64, никаких Buffer в сессии
      // { name: string, data: base64string, caption: string }
      files: [],
      totalSize: 0,
    };
...
reportIssue.on('text', async (ctx) => {
  ...
  // Не перехватываем команды — пусть обрабатывает глобальный /start и т.п.
  ...
  if (text === 'Готово') {
    try {
      ...
      const issueData = {
        ...
        // учитываем и selectedOrganization, и selectedOrg
        company: ctx.session.selectedOrganization || ctx.session.selectedOrg || 'Не указано',
        ...
        files: ctx.session.issueData.files.map((file, index) => {
          ...
          file.newName = newFileName; // на время, чтобы записать на диск
          return {
            name: newFileName,
      ...
      // Сохраняем файлы с новым именем (восстанавливаем Buffer из base64)
      for (let i = 0; i < ctx.session.issueData.files.length; i += 1) {
      ...
      // Полная очистка сессии (без Buffer внутри)
      ctx.session = {
...
reportIssue.on(['photo', 'video', 'video_note', 'voice', 'document'], async (ctx) => {
  try {
    ...
    // Сохраняем файл в сессии как base64 (+ подпись)
    ctx.session.issueData.files.push({
      name: fileName,
      data: base64, // <— base64, не Buffer!
      caption,
    });
...
async function saveTicketFromFolder(telegramId, ticketFolder) {
  try {
    ...
    if (!user) {
      // .returning('*') в некоторых БД (например, MySQL) недоступен — предполагаю PostgreSQL
      user = await db('users')
        .insert({
    ...
    for (const file of issueData.files) {
      ...
      const fileData = await fs.readFile(filePath); // Buffer
      await db('files').insert({
        ...
        size: fileStats.size,
        path: filePath,
        data: fileData, // Buffer для bytea
        created_at: db.fn.now(),
      });
    }
